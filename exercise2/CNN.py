...
class FullyConnectedLayer(Layer):
    ...
    def __init__(self, input, n_units, activation=tf.nn.relu):
        self.weights = tf.Variable(tf.random_normal(shape=[input.shape()[1],n_units], stddev=0.05))
        self.bias = tf.Variable(tf.random_normal(shape=[n_units], stddev=0.05))
...
class ConvolutionalLayer(Layer):
    ...
    def __init__(self, input, filtersize, n_filters, strides=1, padding="SAME", pooling=None, poolingsize=1, activation=tf.nn.relu):
        self.filters = tf.Variable(tf.random_normal([filtersize, filtersize, input.shape()[3], n_filters], stddev=0.05))
        self.bias = tf.Variable(tf.random_normal([n_filters], stddev=0.05))
        self.activation = activation
...
class OutputLayer(Layer):
    ...
    def __init__(self, input, n_outputs, activation=tf.nn.softmax):
        self.weights = tf.Variable(tf.random_normal(shape=[input.shape()[1],n_outputs], stddev=0.05))
        self.bias = tf.Variable(tf.random_normal(shape=[n_outputs], stddev=0.05))
        self.activation = activation
        self.layer = tf.matmul(input.layer, self.weights) + self.bias
        if self.activation is not None:
            self.layer = self.activation(self.layer)
        ...
        self.setAccuracy(tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.layer, axis=1), tf.argmax(self.true_y, axis=1)), "float32")))
...
class Calculation:
    ...
    def accuracy(self, x, y):
        return self.session.run(self.network.last.accuracy, feed_dict={self.network.first.layer: x, self.network.last.true_y: y})
